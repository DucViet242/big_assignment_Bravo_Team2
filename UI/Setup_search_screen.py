...
# Xóa các widget hiện có
    for widget in search_frame.winfo_children():
        widget.destroy()
# Tiêu đề
    ttk.Label(search_frame, text="Tìm kiếm Dự án", font=("Arial", 14, "bold")).pack(pady=10)
# Frame tìm kiếm
    search_input_frame = ttk.Frame(search_frame)
    search_input_frame.pack(fill=tk.X, padx=10, pady=10)
    ttk.Label(search_input_frame, text="Từ khóa:").pack(side=tk.LEFT, padx=5)
    search_entry = ttk.Entry(search_input_frame, width=30)
    search_entry.pack(side=tk.LEFT, padx=5)
    search_button = ttk.Button(search_input_frame, text="Tìm kiếm", command=handle_search_projects)
    search_button.pack(side=tk.LEFT, padx=5)
# Frame kết quả
    results_frame = ttk.LabelFrame(search_frame, text="Kết quả Tìm kiếm")
    results_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
# Tạo Treeview cho kết quả
    columns = ("ID", "Name", "Description")
    search_results_table = ttk.Treeview(results_frame, columns=columns, show="headings")
# Định dạng các cột
    for col in columns:
...
# Thêm thanh cuộn
    scrollbar = ttk.Scrollbar(results_frame, orient=tk.VERTICAL, command=search_results_table.yview)
