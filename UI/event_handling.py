...
from Dataset_metatdata import metadata_processing
import UI.Dasboard as dashboard
from Business_function_CRUD.Add_project import add_project_from_soure_code, add_project_from_file
from Business_function_CRUD.Delete_project import delete_project
from Business_function_CRUD.Find_project import search_projects, get_project_content
from Business_function_CRUD.Update_project import update_project

def refresh_all():
    """Refresh all tabs and synchronization info."""
...
def refresh_delete_list():
    """Refresh the project list on the delete tab."""
    for item in constants.delete_projects_table.get_children():
        constants.delete_projects_table.delete(item)
    success, message, dataset = metadata_processing.read_metadata()
    if success:
        for row in dataset:
...
def refresh_update_list():
    """Refresh the ID list on the update tab."""
    success, message, dataset = metadata_processing.read_metadata()
    ids = []
    if success:
        for row in dataset:
...
def toggle_input_method():
    """Switch between direct input and file input methods."""
    if constants.input_method_var.get() == "direct":
...
def browse_file():
    """Open a file dialog for selecting a source file."""
...
def handle_update_project():
    """Handle the update project button."""
    project_id = constants.update_id_combo.get()
    if not project_id:
        messagebox.showerror("Lỗi", "Vui lòng chọn một ID dự án!")
        return
